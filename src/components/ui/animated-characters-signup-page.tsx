...
function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message === "ACCOUNT_EXISTS" || error.name === "AccountExistsError") {
        setError("An account with this email already exists. You can sign in with your existing account. We'll help you get back in.");
        setTimeout(() => {
          router.push("/login");
        }, 2000);
      } else {
        setError(error.message || "We couldn't create your account right now. Please try again, and we're here to help if you need support.");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setError(error.message || "We couldn't sign you up with Google right now. Please try again, and we're here to help if you need support.");
      setIsLoading(false);
    }
  };
