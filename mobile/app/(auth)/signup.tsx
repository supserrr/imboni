...
export default function Signup() {
  ...
  const preSelectedType = params.userType as UserType | undefined;
  
  const [fullName, setFullName] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [userType] = useState<UserType>(preSelectedType || 'blind');
  ...
  const emailSnapPoints = useMemo(() => ['60%', '75%'], []);
  ...
  const handleOAuthSignIn = async () => {
    try {
      ...
      setLoading(true);
      const result = await AuthService.signInWithGoogle(userType);
      ...
      <View style={styles.container}>
        ...
        >
          <BottomSheetView style={styles.bottomSheetContent}>
            <Text style={styles.bottomSheetTitle}>Create Account</Text>
